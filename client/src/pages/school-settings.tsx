          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                Reward Currency Settings
              </CardTitle>
              <CardDescription>
                Customize your school's reward currency name and define payout values for different achievements.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              <div className="space-y-4">
                <div>
                  <Label className="text-base font-semibold">Currency Name</Label>
                  <p className="text-sm text-gray-500 mt-1">
                    Set a custom name for your school's reward currency (default: "Bear Bucks")
                  </p>
                </div>
                
                <div className="flex gap-2">
                  <Input
                    placeholder="Bear Bucks"
                    value={rewardCurrencyName}
                    onChange={(e) => setRewardCurrencyName(e.target.value)}
                    className="flex-1"
                  />
                  <Button 
                    onClick={handleUpdateRewardCurrency}
                    variant="outline"
                    disabled={!rewardCurrencyName.trim()}
                  >
                    Update Name
                  </Button>
                </div>
                ...
                <div className="text-sm text-gray-600">
                  Current currency name: <span className="font-medium">
                    {formData.customization?.rewardCurrency?.name || 'Bear Bucks'}
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  {Object.entries(formData.customization?.rewardCurrency?.payouts || {}).map(([rewardName, value]) => (
                    <div key={rewardName} className="flex items-center gap-3 p-3 border rounded-lg bg-gray-50">
                      {editingReward === rewardName ? (
                        <div className="flex items-center gap-2 flex-1">
                          ...
                          <Input
                            type="number"
                            placeholder="Amount"
                            value={editingRewardData.value}
                            onChange={(e) => setEditingRewardData({...editingRewardData, value: e.target.value})}
                            className="w-24"
                          />
                          <Button 
                            size="sm" 
                            onClick={() => {
                              handleEditReward(rewardName, editingRewardData.name, editingRewardData.value);
                            }}
                            className="h-8 w-8 p-0"
                          >
                            <CheckCircle className="h-4 w-4" />
                        ...
                        <div className="flex items-center gap-3 flex-1">
                          ...
                          <div className="text-right">
                            <span className="text-sm font-mono bg-blue-100 text-blue-800 px-2 py-1 rounded">
                              {value} {rewardCurrencyName || 'Bear Bucks'}
                            </span>
                          </div>
                          <Button
                            size="sm"
                            variant="ghost"
                            onClick={() => {
                              setEditingReward(rewardName);
                              setEditingRewardData({ name: rewardName, value: value.toString() });
                            }}
                            className="h-8 w-8 p-0"
                          >
                            <Edit2 className="h-4 w-4" />
                          ...
                          <Button
                            size="sm"
                            variant="ghost"
                            onClick={() => handleDeleteReward(rewardName)}
                            className="h-8 w-8 p-0 text-red-500 hover:text-red-700"
                          >
                            <Trash2 className="h-4 w-4" />
                          </Button>
                  ...
                  {Object.keys(formData.customization?.rewardCurrency?.payouts || {}).length === 0 && (
                    <div className="text-center py-8 text-gray-500">
                      <Gift className="h-12 w-12 mx-auto mb-3 text-gray-300" />
                      <p>No reward payouts configured yet</p>
                      <p className="text-sm">Add your first reward below</p>
                    </div>
                  )}
                </div>
                ...
                <div className="flex gap-2">
                  <Input
                    placeholder="Reward name (e.g., Module Completion)"
                    value={newRewardName}
                  ...
                  <Button 
                    ...
                    disabled={!newRewardName.trim() || !newRewardValue.trim()}
                    className="shrink-0"
                  >
                    <Plus className="h-4 w-4 mr-1" />
                    Add Reward
                ...
                <div className="text-xs text-gray-500">
                  Example rewards: "Module Completion" (10), "Assessment Pass" (15), "Daily Login" (5)
                </div>
              </div>
              ...
              <div className="border-t pt-6">
                ...
                >
                  <Save className="h-4 w-4 mr-2" />
                  {updateSchoolMutation.isPending ? 'Saving...' : 'Save Reward Settings'}
                </Button>
              </div>
            </CardContent>
          </Card>
