...
// Teaching strategies generation endpoint
router.post('/generate-teaching-strategies', async (req, res) => {
  try {
    ...
    const prompt = `Generate 6-8 practical teaching strategies for early childhood educators on "${topic}".
    
Difficulty level: ${difficulty}

Create specific, actionable strategies that teachers can implement immediately. Each strategy should be:
- Practical and realistic for classroom use
- Appropriate for ages 3-6
- Include specific examples or activities
- Be engaging and developmentally appropriate

Format as JSON:
{
  "strategies": [
    "Strategy 1: Specific actionable advice with example",
    "Strategy 2: Another practical strategy with details",
    ...
  ]
}`;
    ...
    const content = JSON.parse(response.choices[0].message.content || '{"strategies": []}');
    res.json(content);
  } catch (error) {
    console.error('Error generating teaching strategies:', error);
    res.status(500).json({ 
      error: 'Failed to generate teaching strategies',
      strategies: [`Use hands-on activities to teach ${req.body.topic}`, `Create visual aids for ${req.body.topic} concepts`]
    });
  }
});

// Quiz questions generation endpoint
router.post('/generate-quiz-questions', async (req, res) => {
  try {
    ...
    const prompt = `Generate ${count} multiple choice quiz questions for early childhood educators about "${topic}".
    
Difficulty: ${difficulty}

Each question should test practical knowledge and application. Format as JSON:
{
  "questions": [
    {
    ...
    res.status(500).json({ 
      ...
      questions: [{
        question: `What is an important aspect of ${req.body.topic}?`,
        options: ["Evidence-based practice", "Ignoring guidelines", "Random approaches", "Avoiding training"],
        correctAnswer: 0,
        explanation: "Evidence-based practices ensure effective and safe implementation."
      }]
    });
  }
});

// Assessment questions generation endpoint
router.post('/generate-assessment-questions', async (req, res) => {
  try {
    const { topic, difficulty = 'intermediate' } = req.body;
    ...
    const prompt = `Generate 5 assessment questions for early childhood educators about "${topic}".
    
Difficulty: ${difficulty}

Create questions that assess understanding and application of ${topic} concepts. Mix question types including multiple choice, scenario-based, and practical application questions.
...
Format as JSON:
{
  "assessmentQuestions": [
    ...
    },
    {
      ...
      "sampleAnswer": "Expected response elements"
    }
  ]
}`;
    ...
    const content = JSON.parse(response.choices[0].message.content || '{"assessmentQuestions": []}');
    res.json(content);
  } catch (error) {
    ...
    res.status(500).json({ 
      ...
      assessmentQuestions: [{
        type: "multiple-choice",
        question: `Which approach is most effective for ${req.body.topic}?`,
        options: ["Research-based methods", "Trial and error", "Personal opinion only", "Outdated practices"],
        correctAnswer: 0,
        explanation: "Research-based methods provide the most reliable outcomes."
      }]
    });
  }
});
